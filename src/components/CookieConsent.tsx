...
export default function CookieConsent({
  ...
  useEffect(() => {
    ...
    // Safe check for existing consent
    const consent = SafeStorage.get(CONSENT_KEY);
    if (!consent) {
  ...
  const handleAcceptAll = () => {
    ...
    const success = SafeStorage.setJSON(CONSENT_KEY, consent);
    if (success) {
      setShowBanner(false);
                  ...
                  >
                    <div className="space-y-2 py-3 border-t border-purple-200/50 dark:border-purple-800/50">
                      {[
                        ...
                        { label: '📊 Analytics', desc: 'Help us improve our service', enabled: preferences.analytics, disabled: false, key: 'analytics' },
                        { label: '🎯 Marketing', desc: 'Personalized content & ads', enabled: preferences.marketing, disabled: false, key: 'marketing' }
                      ].map((item, index) => (
                        <motion.label
                          key={index}
